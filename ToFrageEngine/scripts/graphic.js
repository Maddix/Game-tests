		...
		return creation.extend(
			...
			{	
				...
				_add: function(item) {
					...
					if (this.context) item.setup(this);
				},
		...
		return creation.extend(
			{
				...
				context: null,
				renderLayer: null,
				setup: function(renderLayer) {
					this.renderLayer = renderLayer;
					this.context = renderLayer.context;
				}
			},
			config);
	...
	*/
	...
	localContainer.borderedRectangle = function(config) {
		return creation.compose(
			...
			function(rectangle) {
				var rectUpdate = rectangle.updateGraphics;
				rectangle.updateGraphics = function() {
					rectUpdate();
					this.drawBorder();
				};
			},
	...
	localContainer.borderedCircle = function(config) {
		return creation.compose(
			...
			localContainer.border(),
			function(circle) {
				var circleUpdate = circle.updateGraphics;
				circle.updateGraphics = function() {
					circleUpdate();
					this.drawBorder()
				};
			},
